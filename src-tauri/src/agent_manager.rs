...
impl AgentManager {
                ...
                if !manager.check_agent_port().await {
                    if manager.agent_running.load(Ordering::Relaxed) {
                        ...
                        if let Err(e) = manager.start_agent_with_retry().await {
                            ...
                            manager.emit_agent_status("error", Some(&format!("Restart failed: {}", e)));
                        }
                    }
                } else {
                    if !manager.agent_running.load(Ordering::Relaxed) {
    ...
    async fn start_agent_with_retry(&self) -> Result<()> {
        for attempt in 1..=MAX_STARTUP_RETRIES {
            log::info!("[AgentManager] Starting agent (attempt {}/{})", attempt, MAX_STARTUP_RETRIES);
            ...
            match self.start_agent_once().await {
                ...
                Err(e) => {
                    ...
                    if attempt < MAX_STARTUP_RETRIES {
                        log::info!("[AgentManager] Waiting {} seconds before retry", RETRY_DELAY_SECONDS);
        ...
        Err(anyhow!("Failed to start agent after {} attempts", MAX_STARTUP_RETRIES))
    }
    ...
    async fn start_agent_once(&self) -> Result<()> {
        ...
        if let Some(process_name) = self.check_agent_port_with_process().await {
            if process_name == "agent" || process_name == "agent.exe" {
                ...
                return Err(anyhow!("Port {} is occupied by {}", AGENT_PORT, process_name));
            }
        }
    ...
    async fn wait_for_agent_startup(&self, app_data_dir: &PathBuf) -> Result<()> {
        ...
        Err(anyhow!("Agent startup timeout after {} seconds", STARTUP_TIMEOUT_SECONDS))
    }
    ...
    fn emit_agent_status(&self, status: &str, message: Option<&str>) {
        let _ = self.app.emit_all("agent_manager_status", serde_json::json!({
            "status": status,
            "message": message,
